...
if __name__ == '__main__':
    ...
    filename = "sarcoidose_experiment_3_all"
    ...
    mfs = LinearSVC(C=1, penalty="l2", dual=False, max_iter=5000)
...
#        estimators.append(('ponyGE', LogisticRegression()))
        estimators.append(('ponyGE', ponyge(RANDOM_SEED=seed, MAX_INIT_TREE_DEPTH=5, MAX_TREE_DEPTH=5)))
        model = Pipeline(estimators)

        param_grid = {
            'fs__n_features_to_select': [12,18,24],
            'ponyGE__GENERATIONS': [50, 100, 200],
            'ponyGE__POPULATION_SIZE': [100, 200, 500, 1000, 3000]

        }

        options = {'Importance'}#{'ponyge'}
