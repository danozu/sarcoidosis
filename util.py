...
def evaluate_grid_model(X,Y,crossval_index, model,param_grid,scoring,num_folds,seed,options,num_class):
    ...
    for i in range(experiments):
        ...
        grid = GridSearchCV(estimator=model, param_grid=param_grid, scoring=scoring, cv=kfold, refit = 'True', n_jobs=10)
        ...
        if ('pgliq' in options):
            ...
            genotype = grid_result.best_estimator_.named_steps['PGLIQ_APF']._final_genotype
            print("Length of Final Individual: %s" % (str(length)[1:-1]))
            print("Final Genotype: %s" % genotype)
            print("Final Phenotype: %s" %lisp)
