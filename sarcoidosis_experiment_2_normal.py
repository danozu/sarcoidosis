...
if __name__ == '__main__':
    ...
    filename = "sarcoidosis_experiment_3_normal"
    ...
    if Run == 1:
        ...
        ponyge_grid_result, ponyge_class, ponyge_auc, ponyge_probs, ponyge_preds, ponyge_score, ponyge_params, ponyge_features, ponyge_cc = evaluate_grid_model(
            X, Y, crossval_index, model, param_grid, scoring, num_folds, seed, options, num_class)

        classes = functools.reduce(operator.iconcat, ponyge_class, [])
        y_probs = functools.reduce(operator.iconcat, ponyge_probs, [])  # flatten the list of lists
        fpr, tpr, thresholds = metrics.roc_curve(classes, y_probs, drop_intermediate=False)
        print(f'AUC = {metrics.auc(fpr, tpr)}')

        # Store the result in dataframe
        
        class_df = pd.DataFrame(columns=['BFP', 'SVM', 'KNN', 'ADAB', 'RF', 'LGB', 'XGB', 'DT', 'LOGR', 'GP', 'ponyGE'])
        ...
        class_df['ponyGE'] = functools.reduce(operator.iconcat, ponyge_class, [])
        ...
        params_df['ponyGE'] = pd.Series(ponyge_params)  # Transform to series

        params_df.to_csv(filename_params, index=False)

        cc_df = pd.DataFrame(columns=['ponyGE'])

        cc_df['ponyGE'] = functools.reduce(operator.iconcat, ponyge_cc, [])  # Transform to series
        ...
        result_df['ponyGE'] = functools.reduce(operator.iconcat, ponyge_probs, [])  # flatten the list of lists
        result_df['class'] = functools.reduce(operator.iconcat, ponyge_class, [])  # flatten the list of lists
